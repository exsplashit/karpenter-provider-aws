...
build: ## Build the Karpenter controller and webhook images using ko build
	$(eval CONTROLLER_IMG=$(shell $(WITH_GOFLAGS) ko build -B github.com/exsplashit/karpenter-provider-aws/cmd/controller))
	$(eval WEBHOOK_IMG=$(shell $(WITH_GOFLAGS) ko build -B github.com/exsplashit/karpenter-provider-aws/cmd/webhook))
